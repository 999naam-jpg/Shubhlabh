        ...
        <div className={styles.overlay} onClick={() => setModal(null)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <div>
                <h3>{modal === 'add' ? '➕ Add Backdrop' : '✏️ Edit Backdrop'}</h3>
            ...
            {saveError && <p style={{ color: '#dc2626', fontSize: '0.8rem', padding: '0 1rem' }}>{saveError}</p>}
            <div className={styles.modalFooter}>
              <button className={styles.cancelBtn} onClick={() => setModal(null)}>Cancel</button>
              <button className={styles.saveBtn} onClick={handleSave} disabled={saving}>{saving ? 'Saving...' : modal === 'add' ? '➕ Add Backdrop' : '💾 Save Changes'}</button>
            </div>
          </div>
        </div>
      )}
