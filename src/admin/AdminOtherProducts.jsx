...
const empty = { name: '', price: '', image: '', stock: 'Available', quantity: '', trending: false, desc: '', photos: [] }
...
export default function AdminOtherProducts() {
  ...
  const openEdit = (p) => { setForm({ ...p, photos: p.photos || [] }); setModal(p) }

  const handleSave = async () => {
    if (!form.name || !form.price) return
    try {
      if (modal === 'add') {
        const saved = await api.createOtherProduct(form)
        setItems(prev => [...prev, saved])
      } else {
        const updated = await api.updateOtherProduct(modal._id, form)
  ...
  const handleDelete = async (p) => {
    ...
    await api.deleteOtherProduct(p._id)
  ...
  return (
    <div>
      <div className={styles.topRow}>
        <h2 className={styles.heading}>📦 Other Products</h2>
      ...
      {loading ? <div style={{ textAlign: 'center', padding: '3rem', color: '#aaa' }}>Loading...</div> : (
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            <thead><tr><th>Item</th><th>Price</th><th>Stock</th><th>Qty</th><th>Photos</th><th>Trending</th><th>Actions</th></tr></thead>
            <tbody>
              {filtered.map(p => (
                <tr key={p._id}>
                  <td><div className={styles.productCell}><img src={p.image || p.photos?.[0]} alt={p.name} /><span>{p.name}</span></div></td>
                  ...
                  <td><span className={styles.qty}>{p.photos?.length || 0}</span></td>
                  <td>{p.trending ? '🔥' : '—'}</td>
      ...
      {modal && (
        <div className={styles.overlay} onClick={() => setModal(null)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            <h3>{modal === 'add' ? 'Add Other Product' : 'Edit Other Product'}</h3>
            <div className={styles.formGrid}>
              ...
              <div className={styles.formField}><label>Price *</label><input value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} placeholder="e.g. ₹500/day" /></div>
            ...
            <div className={styles.formField}><label>Description</label>
              <textarea value={form.desc || ''} onChange={e => setForm({ ...form, desc: e.target.value })} rows={2}
                style={{ padding: '0.6rem', borderRadius: 8, border: '1.5px solid #e5e7eb', fontFamily: 'inherit', fontSize: '0.88rem', resize: 'vertical', outline: 'none', width: '100%' }} />
            </div>
